...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erros)) {
        try {
            $stmt = $pdo->prepare("
                INSERT INTO produtos (nome, descricao, preco, estoque, categoria_id, destaque, ativo) 
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ");
            ...
            header('Location: listar.php?msg=sucesso&texto=Produto cadastrado com sucesso');
            exit;
        } catch (PDOException $e) {
            $erros['geral'] = 'Erro ao cadastrar produto: ' . $e->getMessage();
        }
    }
}
        ...
        <form method="POST" action="" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="preco" name="preco" value="<?php echo htmlspecialchars($produto['preco']); ?>" 
